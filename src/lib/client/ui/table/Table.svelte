			...
			{#each sorted as row, idx}
				...
				>
					{#if expanded}
						<td class="px-2 py-3 text-center">
							<button
								type="button"
								class="inline-flex items-center justify-center rounded-control-sm p-1 text-text-muted hover:text-text hover:bg-surface-hover transition-colors"
								onclick={(e) => { e.stopPropagation(); toggleExpand(idx); }}
							>
								<ChevronRight
									size={14}
									class="transition-transform {expandedRows.has(idx) ? 'rotate-90' : ''}"
								/>
							</button>
						</td>
					{/if}
				...
				{#if expanded && expandedRows.has(idx)}
					<tr class="border-b border-border-subtle last:border-b-0">
						<td colspan={colCount} class="bg-surface-muted px-4 py-4">
							{@render expanded(row)}
						</td>
					</tr>
				{/if}
