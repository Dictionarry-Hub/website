          ...
          {navItems.map((item) => (
            ...
            >
              <div className="relative z-10 flex items-center">
                <span className="transition-transform duration-200 group-hover:-translate-y-0.5">{item.label}</span>
              </div>
              {isActiveRoute(item.href) && (
                <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-4/5 h-0.5 bg-blue-600 dark:bg-blue-400 rounded-full transition-all duration-300" />
              )}
              <div className="absolute inset-0 bg-gray-100 dark:bg-gray-700 opacity-0 group-hover:opacity-100 transition-opacity duration-200 rounded-lg" />
            </Link>
          ))}
        </div>
