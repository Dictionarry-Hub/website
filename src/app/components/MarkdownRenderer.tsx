  ...
  return (
    <ReactMarkdown
      ...
      components={{
        ...
        tr({ children, ...props }) {
          return (
            <tr className="border-t border-gray-200 dark:border-gray-700 first:border-0" {...props}>
              {children}
            </tr>
          );
        },
        th({ children, ...props }) {
          return (
            <th className="p-2 text-sm font-medium text-gray-900 dark:text-gray-100" {...props}>
              {children}
            </th>
          );
        },
        td({ children, ...props }) {
          ...
          return (
            <td className="p-2 text-sm text-gray-700 dark:text-gray-300 align-top" {...props}>
        ...
        img({ src, alt = '', width, height, ...props }: React.ImgHTMLAttributes<HTMLImageElement>) {
          ...
          if (isVideo) {
            const shouldRender =
              (isDarkMode && isDarkAlt) || (!isDarkMode && isLightAlt) || (!isDarkAlt && !isLightAlt);
        ...
        pre({ children, ...props }: React.PropsWithChildren<React.HTMLAttributes<HTMLPreElement>>) {
          ...
          if (React.isValidElement(children)) {
            // Type assertion to address TypeScript issue
            ...
            // Manually set language to yaml for docker compose examples
            if (
              language === '' &&
              typeof childProps.children === 'string' &&
              (childProps.children.includes('services:') || childProps.children.includes('container_name:'))
            ) {
              language = 'yaml';
            }
          ...
          return (
            <pre
              className="!mt-6 !mb-6 rounded-lg !bg-[#f7fafc] dark:!bg-[#1a202c] !border !border-gray-200 dark:!border-gray-700 overflow-x-auto shadow-md"
              style={{
                padding: '1.25rem',
                lineHeight: 1.5,
                fontSize: '0.95rem',
        ...
        code({ inline, className, children, ...props }: CodeProps) {
          // Convert children to string and handle any edge cases
          const childrenContent = String(children).replace(/\n$/, '');

          if (inline) {
            // Handle inline code - we don't need to worry about backticks here
            // ReactMarkdown already handles that at the parser level
            return (
          ...
          if (!lang && childrenContent.includes('services:') && childrenContent.includes('container_name:')) {
            lang = 'yaml';
          } else if (!lang && childrenContent.includes('docker run') && childrenContent.includes('-v ')) {
            lang = 'bash';
          }

          const highlighted =
            lang && Prism.languages[lang]
              ? Prism.highlight(childrenContent, Prism.languages[lang], lang)
              : childrenContent;
        ...
        hr(props: React.HTMLAttributes<HTMLHRElement>) {
          return <hr className="my-4 border-t border-gray-200 dark:border-gray-700" {...props} />;
        },
