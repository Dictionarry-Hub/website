...
import { regularExpressionToYaml } from '$lib/shared/utils/yaml/index.js';
...
export const GET: RequestHandler = ({ params }) => {
	...
	const data = entry[1];
	const regex = data.regularExpressions.find((item) => slugify(item.name) === params.slug);
	if (!regex) error(404, 'Regular expression not found');

	return new Response(regularExpressionToYaml(data, regex), {
		headers: {
