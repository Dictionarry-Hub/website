...
<button
  class="relative flex items-center justify-center {isPortrait ? 'gap-1' : 'gap-2'} {isPortrait ? 'h-7 text-[10px]' : 'w-auto text-[11px]'} {isPortrait ? 'px-2' : 'px-3 py-1.5'} {isPortrait ? 'rounded border' : 'rounded-md border-2'} transition-all z-10 font-mono
    {isSelected 
      ...
      : isEnabled
        ? 'bg-white dark:bg-neutral-900 text-neutral-700 dark:text-neutral-300 border-neutral-300 dark:border-neutral-700 hover:border-neutral-500 dark:hover:border-neutral-500 hover:bg-neutral-50 dark:hover:bg-neutral-800/50 cursor-pointer'
