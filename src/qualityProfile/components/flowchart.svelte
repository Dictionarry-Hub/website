    ...
    <div class="relative grid {state.currentColumn === 1 ? 'grid-cols-1' : state.currentColumn === 2 ? 'grid-cols-2' : state.currentColumn === 3 ? 'grid-cols-3' : state.currentColumn === 4 ? 'grid-cols-4' : 'grid-cols-5'}">
      {#each [1, 2, 3, 4, 5] as column}
        {#if column <= state.currentColumn}
          <div class="p-4 {column < state.currentColumn ? 'border-r' : ''} border-neutral-200 dark:border-neutral-700">
            <div class="flex flex-col items-center space-y-3">
              {#each [1, 2, 3] as button}
                <button
                  bind:this={buttonRefs[getButtonRef(column, button)]}
                  class="relative w-32 max-w-[60%] px-4 py-3 text-sm font-medium rounded-lg border transition-all z-10
